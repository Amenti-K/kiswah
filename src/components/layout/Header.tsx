...
const Header = () => {
  const { data: companyInfo, status } = useAppSelector(
  ...
  const [showMainHeader, setShowMainHeader] = useState(() => {
    if (typeof window !== "undefined") {
      return !!localStorage.getItem("kiswah_bootstrapped");
    }
    return false;
  });
  ...
  useEffect(() => {
    if (status === "succeeded") {
      localStorage.setItem("kiswah_bootstrapped", "true");
      if (isFirstSessionLoad) {
        const timer = setTimeout(() => setShowMainHeader(true), 800);
        return () => clearTimeout(timer);
      } else {
        setShowMainHeader(true);
      }
    }
  }, [status, isFirstSessionLoad]);
  ...
  return (
    <>
      <AnimatePresence>
        {isFirstSessionLoad && status !== "succeeded" && (
          <InitialLoader
            companyName={companyInfo?.company.companyName}
            logoUrl={companyInfo?.company.logoUrl}
          />
        )}
      </AnimatePresence>
      ...
      <motion.header
        ...
        animate={
          showMainHeader ? { y: 0, opacity: 1 } : { y: -100, opacity: 0 }
        }
                  ...
                  {tools.map((tool, index) =>
                    tool.children ? (
                      ...
                      <li key={index}>
                        ...
                        >
                          {tool.label}
                          {/* (Optional) Add Chevron or something to indicate it's a file? No. */}
                        </Link>
                      </li>
                    ),
                  )}
